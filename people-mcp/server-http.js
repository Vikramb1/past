...
function parseNaturalLanguageQuery(query) {
  ...
  if (lowerQuery.includes('works at') || lowerQuery.includes('from company') || lowerQuery.includes('company')) {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
      tools: [
        {
          name: "search_people",
          description: "Search for people using natural language queries",
          inputSchema: {
            type: "object",
            properties: {
              query: {
                type: "string",
                description: "Natural language query to search for people"
              }
            },
            required: ["query"]
          }
        ...
        },
        {
          ...
          description: "Find a person by their image",
          inputSchema: {
            type: "object",
            properties: {
              email: {
...
// Simple REST API endpoints for direct access
...
app.get('/api/person/:email', async (req, res) => {
  ...
  res.json({
    ...
    endpoints: {
      ...
      rest_api: {
        ...
        person_details: 'GET /api/person/:email',
        list_people: 'GET /api/people'
      }
    }
  });
});
...
app.get('/', (req, res) => {
  res.send(`
    <h1>People MCP Server</h1>
    <p>Running on port ${PORT}</p>

    <h2>MCP Endpoints:</h2>
    <ul>
      <li><strong>SSE Transport:</strong> GET /sse</li>
    </ul>

    <h2>REST API Endpoints:</h2>
    <ul>
      <li><strong>Search:</strong> POST /api/search - Body: {"query": "search term"}</li>
      <li><strong>Person Details:</strong> GET /api/person/:email</li>
      <li><strong>List People:</strong> GET /api/people?limit=10</li>
      <li><strong>Health Check:</strong> GET /health</li>
    </ul>

    <h2>Test with Online MCP Inspector:</h2>
    <p>
      1. Go to the MCP Inspector<br>
      2. Select "SSE" transport<br>
      3. Enter URL: http://localhost:${PORT}/sse<br>
      4. Click Connect
    </p>
...
# Get person details
curl http://localhost:${PORT}/api/person/sanjayamirthraj@example.com

# List all people
curl http://localhost:${PORT}/api/people?limit=5
    </pre>
  `);
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`\n🚀 People MCP Server running on http://localhost:${PORT}`);
  console.log(`\n📡 Endpoints:`);
